...
import { BuyingPower, ChartData, News, PriceData, Stock, Suggestion, UserBalance, UserStockTable } from '../interfaces';
@Injectable({
  providedIn: 'root'
})
export class StockServiceService {
  ...
  getCurrentStockPrice(ticker:string):Observable<PriceData>{
    return this.http.get(`${environment.REALTIME_URL}?ticker=${ticker}`,{withCredentials:true}) as Observable<PriceData>
  }
  getChartData(ticker:string, timeFrame:string):Observable<Array<ChartData>>{
    return this.http.get(`${environment.CHART_URL}?ticker=${ticker}&timeFrame=${timeFrame}`,{withCredentials:true}) as Observable<Array<ChartData>>
  }
